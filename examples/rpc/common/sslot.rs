...
use super::{
  constant::{INVALID_REQ_TYPE, SESSION_CREDITS},
  msgbuffer::MsgBuffer,
  rpc::Tag,
  transport::RoutingInfo,
};
...
impl SSlot {
  ...
  pub fn client_info_mut(&mut self) -> Option<&mut ClientInfo> {
    match self.info {
      Info::Client(ref mut client) => Some(client),
      _ => None,
    }
  }
  ...
  pub fn server_info_mut(&mut self) -> Option<&mut ServerInfo> {
    match self.info {
      Info::Server(ref mut server) => Some(server),
      _ => None,
    }
  }
  ...
  pub fn client_info(&self) -> Option<&ClientInfo> {
    match self.info {
      Info::Client(ref client) => Some(client),
      _ => None,
    }
  }
  ...
  pub fn server_info(&self) -> Option<&ServerInfo> {
    match self.info {
      Info::Server(ref server) => Some(server),
      _ => None,
    }
  }
