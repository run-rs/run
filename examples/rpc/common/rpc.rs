use std::{ collections::{VecDeque}, ptr::NonNull, cell::RefCell, rc::Rc};
...
use crate::common::{constant::{SESSION_CREDITS, MAX_MSG_SIZE, TTR_MAX_DATA_PER_PKT, INVALID_REQ_TYPE}, msgbuffer::{RPC_HEADER_LEN, HEADER_LEN}, sslot::{ClientInfo, Info}, time::to_usec};
...
impl ReqHandle  {
    pub fn get_req_msgbuf(&self)->MsgBuffer{
        self.0.borrow().server_info().unwrap().req_msgbuf.as_ref().unwrap().clone()
    }
...
impl Rpc {
    ...
    fn pkt_loss_scan_st(&mut self){
        ...
        let mut cur =self.active_rpcs_head_sentinel.borrow().client_info().unwrap().next.clone();
        loop {
            if cur.is_none() {
                break;
            }
            ...
            if self.ev_loop_tsc - sslot.borrow().client_info().unwrap().progress_tsc > self.rpc_rto_cycles {
    ...
    fn pkt_loss_retransmit_st(&mut self,sslot:Rc<RefCell<SSlot>>){
        ...
        let delta=sslot.borrow().client_info().unwrap().num_tx-sslot.borrow().client_info().unwrap().num_rx;
        assert!(self.credits as usize+delta<=SESSION_CREDITS);

        if delta == 0 {
            trace!(
                "False positive. Ingoring"
            );
            return;
        }
    ...
    fn kick_rfr_st(&mut self,sslot:Rc<RefCell<SSlot>>){
        ...
        assert!(sslot.borrow().client_info().unwrap().num_rx >= sslot.borrow().tx_msgbuf.as_ref().unwrap().num_pkts());
        let wire_pkts=sslot.borrow().tx_msgbuf.as_ref().unwrap().num_pkts()
        +
        sslot.borrow().client_info().unwrap().resp_msgbuf.as_ref().unwrap().num_pkts()
        -1;
        ...
        for _ in 0..sending {
            self.enqueue_rfr_st(sslot.clone(),&sslot.borrow().client_info().unwrap().resp_msgbuf.as_ref().unwrap().get_pkthdr_0());
    ...
    fn enqueue_hdr_tx_burst_st(&mut self,_sslot:Rc<RefCell<SSlot>>,ctrl_msgbuf:MsgBuffer){
    ...
    fn req_pkts_pending(&self,sslot:Rc<RefCell<SSlot>>)->bool{
        sslot.borrow().client_info().unwrap().num_tx < sslot.borrow().tx_msgbuf.as_ref().unwrap().num_pkts()
    }
    ...
    fn process_comps_st(&mut self){
        ...
        for mut mbuf in rx_ring.drain(..) {
            ...
            if epkt.dest_mac() != self.local_mac {
                trace!("ethernet packet dest mac {} is not here {}. Drop it",epkt.dest_mac(),self.local_mac);
                continue;
            }
            ...
            if ippkt.dest_ip() != self.local_ipv4 {
                trace!("ipv4 packet dest ip {} is not here {}. Drop it",ippkt.dest_ip(),self.local_ipv4);
                continue;
            }
            ...
            if pkt_hdr.magic() != 11 {
                trace!(
                    "Received a packet with invalid magic.Dropping"
                );
                continue;
            }
    ...
    fn process_small_req_st(&mut self,sslot:Rc<RefCell<SSlot>>,pkt_hdr:&RpcHeader){
        ...
        if pkt_hdr.req_num() <= sslot.borrow().cur_req_num as u64 {
            ...
            if pkt_hdr.req_num() < sslot.borrow().cur_req_num as u64 {
                ...
                trace!(
                    "Response not available yet. Dropping."
                );
                return;
            }
        }
        ...
        assert!(pkt_hdr.req_num() == sslot.borrow().cur_req_num as u64+SESSION_REQ_WINDOW as u64);
        ...
        assert!(sslot.borrow().server_info().unwrap().req_type as usize == INVALID_REQ_TYPE);
        sslot.borrow_mut().server_info_mut().unwrap().req_type=pkt_hdr.req_type() as u8;
        sslot.borrow_mut().server_info_mut().unwrap().req_msgbuf=Some(
            unsafe{
                MsgBuffer::from_pkthdr(pkt_hdr,pkt_hdr.msg_size() as usize)
            }
        );

        (req_func.unwrap().req_func)(ReqHandle::from(sslot.clone()),self.context.clone());
    ...
    fn process_resp_one_st(&mut self,sslot:Rc<RefCell<SSlot>>,pkt_hdr:&RpcHeader,mut payload:Pbuf,_rx_tsc:u64){
        ...
        //else{
            ...
            sslot.borrow_mut().client_info_mut().unwrap()
                .resp_msgbuf.as_mut().unwrap()
        ...
        if !self.enq_req_backlog.is_empty() {
            ...
            self.enqueue_request(args.req_type, 
                args.req_msgbuf, 
                args.resp_msgbuf, 
                args.cont_func, 
                args.tag
        ...
        (cont_func)(sslot.borrow().client_info().unwrap()
        .resp_msgbuf.as_ref().unwrap().clone());
    }
    ...
    fn delete_from_active_rpc_list(&mut self,sslot:Rc<RefCell<SSlot>>){
        ...
        next.as_mut().unwrap().borrow_mut().client_info_mut().unwrap().prev=prev.clone();
        prev.unwrap().borrow_mut().client_info_mut().unwrap().next=next;
    }

    #[inline]
    fn in_order_client(&mut self,sslot:Rc<RefCell<SSlot>>,pkt_hdr:&RpcHeader)->bool{
        if pkt_hdr.req_num() != sslot.borrow().cur_req_num as u64 {
            return false;
        }
    ...
    pub fn enqueue_response(&mut self,sslot:Rc<RefCell<SSlot>>){
        ...
        let num_pkts=sslot.borrow().server_info().unwrap().req_msgbuf.as_ref().unwrap().num_pkts();
        assert!(num_pkts==1);
        ...
        assert_eq!(pkt_hdr.magic(),11);
        assert_eq!(pkt_hdr.pkt_num(),sslot.borrow().server_info().unwrap().sav_num_req_pkts as u16 - 1);
        ...
        assert!(sslot.borrow().server_info().unwrap().req_type as usize!=INVALID_REQ_TYPE);
        sslot.borrow_mut().server_info_mut().unwrap().req_type=INVALID_REQ_TYPE as u8;
    ...
    fn add_to_active_rpc_list(&self,sslot:Rc<RefCell<SSlot>>){
        let prev_tail=self.active_rpcs_tail_sentinel
            .borrow_mut()
            .client_info_mut()
            .unwrap()
            .prev.clone().unwrap();
        prev_tail.borrow_mut()
                .client_info_mut()
                .unwrap()
                .next = Some(sslot.clone());
        sslot.borrow_mut()
             .client_info_mut()
             .unwrap()
             .prev = Some(prev_tail);
        ...
        sslot.borrow_mut()
            ...
            .unwrap()
            .next = Some(
                self.active_rpcs_tail_sentinel.clone()
            );

        self.active_rpcs_tail_sentinel
            .borrow_mut()
            .client_info_mut()
            .unwrap()
            .prev=Some(sslot);
    ...
    fn enqueue_pkt_tx_burst_st(&mut self,sslot:Rc<RefCell<SSlot>>,pkt_idx:usize){
        let item =TxBurstItem{
    ...
    fn do_tx_burst_st(&mut self){
        ...
        */
        ...
        for item in self.tx_burst_batch.drain(..) {
            ...
            let mut ippkt = Ipv4Packet::prepend_header(udp_pkt.release(), &IPV4_HEADER_TEMPLATE);
