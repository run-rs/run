...
use crate::{transport::{MemRegInfo, RegMrFunc, DeRegMrFunc}, utils::SlowRand, constant::HUGE_PAGE_SIZE};

use libc::{shmdt, c_void, shmget, IPC_CREAT, IPC_EXCL, SHM_HUGETLB, shmat, shmctl, IPC_RMID, MPOL_BIND};
...
impl HugeAlloctor{
    pub fn new(
        ...
        Self { 
            ...
            pre_allocation_size:core::cmp::max(initial_size, HugeAlloctor::MAX_CLASS_SIZE),
            stats: Stats::new() 
        }
    }
    ...
    fn alloc_raw(&mut self,size:usize,do_register:bool)->Result<Buffer,HugeAlloctError>{
        ...
        loop {
            ...
            if shm_id == -1 {
                ...
                match errno {
                    ...
                    _ => {
                        log::error!(
                            "HugeAlloc:Unexpected SHM malloc error {}",
                            errno
                        );
        ...
        let shm_buf=unsafe{
            shmat(shm_id, std::ptr::null(), 0)
        };
        if shm_buf.is_null() {
            log::error!(
                "HugeAlloc:shmat() failed. Key={}",
                shm_key
            );
        ...
        let ret =unsafe{
            mbind(shm_buf, size as u64, MPOL_BIND, &nodemask as *const u64, 32, 0)
        };
        if ret!=0 {
            log::error!(
                "HugeAlloc:mbind() failed.Key= {}",
                shm_key
            );
        ...
        if do_register {
            reg_info=(self.reg_mr_func)(Some(unsafe{NonNull::new_unchecked(shm_buf)}),size);
        }
    ...
    fn reserve_hugepages(&mut self)->bool{
        ...
        if let Ok(buffer) = self.alloc_raw(self.pre_allocation_size, true){
            ...
            for i in 0..num_buffers {
                let buf = unsafe{
                    buf_start.as_ptr().add(i*HugeAlloctor::MAX_CLASS_SIZE)
                };
                self.freelist[HugeAlloctor::NUM_CLASSES-1]
                    .push(
                        Buffer{
    ...
    fn drop(&mut self) {
        ...
        for shm_region in self.shm_list.iter_mut() {
            ...
            if shm_region.buf.is_none() {
                log::error!("The buffer start address of Shared Memory Region is Null,\
                SHM buf key:{}",shm_region.shm_key);
                std::process::exit(-1);
            }
            #[cfg(target_os="linux")]
            {
                let ret = unsafe{
                    shmdt(shm_region.buf.unwrap().as_ptr() as *const c_void)
                };

                if ret !=0 {
                    log::error!("HugeAlloc: Error freeing SHM buf for key {}",shm_region.shm_key);
                    std::process::exit(-1);
                }
            }
        }
...
impl Stats{
    pub fn new()->Self{
        Self { shm_reserved: 0, user_alloc_tot: 0 }
    }
}
...
impl HugeAlloctor{
    pub const ALLOC_FAIL_HELP_STR:&'static str="This could be due to insufficient huge pages or SHM limits.";
