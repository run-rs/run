...
pub fn measure_rdtsc_freq()->f64{
    ...
    assert!(sum == 13580802877818827968, "Error in RDTSC freq measurement");
    let rdtsc_cycles = rdtsc() -rdtsc_start;
    let freq_ghz = (rdtsc_cycles as f64 *1.0) / std::time::Instant::duration_since(&std::time::Instant::now(), time_start).as_nanos() as f64;
