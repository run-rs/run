...
impl SocketBuffer {
  ...
  fn dequeue_many(&mut self,buf:&mut [u8]) -> usize {
    ...
    unsafe {
      std::ptr::copy_nonoverlapping(self.ptr.as_ptr().add(self.read_at % self.cap),
                                buf.as_mut_ptr(),
                                  len);
    };
    self.length -= len;
...
impl SocketBuffer {
  pub fn write_unallocated(&mut self,mut offset:usize,data:&[u8]) -> usize {
    ...
    while remianing_bytes != 0 {
      ...
      unsafe {
        std::ptr::copy_nonoverlapping(data.as_ptr().add(can_write - remianing_bytes), 
                                 self.ptr.as_ptr().add(start_at), size);
      }
  ...
  pub fn read_allocated(&mut self,mut offset:usize,data:&mut [u8]) -> usize {
    ...
    while remianing_bytes != 0 {
      ...
      let size = std::cmp::min(self.cap - start_at,remianing_bytes);
      unsafe {
        std::ptr::copy_nonoverlapping(self.ptr.as_ptr().add(start_at),
