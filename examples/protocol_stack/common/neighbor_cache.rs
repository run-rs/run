...
mod test {
  ...
  fn test_fill() {
    ...
    assert_eq!(cache.lookup(MOCK_IP_A, now), Answer::Found(HADDR_A));

    assert_eq!(cache.lookup(MOCK_IP_B, now), Answer::Found(HADDR_B))
  }
  ...
  fn test_expire() {
    ...
    assert!(cache
      .lookup(
        MOCK_IP_A,
        Instant::from_millis(0) + Cache::ENTRY_LIFETIME
          - Duration::from_millis(1)
      )
      .found());
    assert!(!cache
      .lookup(MOCK_IP_A, Instant::from_millis(0) + Cache::ENTRY_LIFETIME)
      .found());
    assert!(!cache
      .lookup(MOCK_IP_A, Instant::from_millis(1) + Cache::ENTRY_LIFETIME)
      .found());
  }
  ...
  fn test_flush() {
    ...
    assert!(!cache.lookup(MOCK_IP_B, Instant::from_millis(0)).found());
