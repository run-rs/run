...
static FRAME_BYTES: [u8; 200] = [
  ...
  ,
];

#[allow(dead_code)]
fn build_tcp_tso<'a>(mbuf: &mut Mbuf, payload_len: usize) {
  let total_header_len =
    ETHER_HEADER_LEN + IPV4_HEADER_LEN + TCP_HEADER_LEN + 12;
  unsafe {
    mbuf.extend_front(total_header_len);
  }

  let mut pkt = Pbuf::new(mbuf);
  ...
  of_flag.set_l4_len((TCP_HEADER_LEN + 12) as u64);
...
fn main() {
  ...
  for i in 0..nb_qs {
    ...
    let jh = std::thread::spawn(move || {
      ...
      let payload = vec![0; mss];
      while run.load(Ordering::Acquire) {
        std::thread::sleep(std::time::Duration::from_secs(1));
        let mut mbuf = Mbuf::from_slice(&payload[..], &mp).unwrap();
        build_tcp_tso(&mut mbuf, mss);
