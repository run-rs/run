...
def plot_tcp_csum():
    ...
    with open("./data/tcp_csum.csv", newline = '') as csvfile:
        ...
        for row in reader:
            if row["framework"] == "Pbuf":
                pbuf.append(float(row["throughput"]))
            if row["framework"] == "Cursor":
                cursor.append(float(row["throughput"]))
            if row["framework"] == "SmolTcp":
                smol.append(float(row["throughput"]))
        
        print(len(pbuf))
        print(len(cursor))
        print(len(smol))

    sample_num = min(min(len(pbuf),len(cursor)),len(smol))
    x = np.arange(0,sample_num)
    y_pbuf = []
    y_smol = []
    y_cursor = []
    for idx in x:
        y_pbuf.append(pbuf[idx])
        y_smol.append(smol[idx])
        y_cursor.append(cursor[idx])
    y_pbuf = np.array(y_pbuf)
    y_smol = np.array(y_smol)
    y_cursor = np.array(y_cursor)    

    fig, ax = plt.subplots(constrained_layout = False)
    ax.plot(x,y_pbuf,label = "Pbuf")
    ax.plot(x,y_smol,label = "SmolTcp")
    ax.plot(x,y_cursor, label = "Cursor")
    ...
    plt.savefig("./figures/tcp_rt" + '.png', dpi = 300)
    plt.savefig("./figures/tcp_rt" + '.pdf', dpi = 300)
    plt.savefig("./figures/tcp_rt" + '.svg', dpi = 300)
    plt.savefig("./figures/tcp_rt" + '.jpg', dpi = 300)
