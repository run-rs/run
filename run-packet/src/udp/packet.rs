...
impl<T: PktMut> UdpPacket<T> {
  ...
  pub fn prepend_header<TH: AsRef<[u8]>>(
    ...
    udppkt.set_packet_len_unchecked(
      u16::try_from(udppkt.buf().remaining()).unwrap(),
    );
    udppkt
  }
}
...
mod test_udppkt {
  ...
  static FRAME_BYTES: [u8; 110] = [
    0x00, 0x0b, 0x86, 0x64, 0x8b, 0xa0, 0x00, 0x50, 0x56, 0xae, 0x76, 0xf5,
    0x08, 0x00, 0x45, 0x00, 0x00, 0x5e, 0x5c, 0x65, 0x00, 0x00, 0x80, 0x11,
    0x00, 0x00, 0xc0, 0xa8, 0x1d, 0x3a, 0xc0, 0xa8, 0x1d, 0xa0, 0xeb, 0xd8,
    0x00, 0xa1, 0x00, 0x4a, 0xbc, 0x86, 0x30, 0x40, 0x02, 0x01, 0x03, 0x30,
    0x0f, 0x02, 0x03, 0x00, 0x91, 0xc8, 0x02, 0x02, 0x05, 0xdc, 0x04, 0x01,
    0x04, 0x02, 0x01, 0x03, 0x04, 0x15, 0x30, 0x13, 0x04, 0x00, 0x02, 0x01,
    0x00, 0x02, 0x01, 0x00, 0x04, 0x05, 0x61, 0x64, 0x6d, 0x69, 0x6e, 0x04,
    0x00, 0x04, 0x00, 0x30, 0x13, 0x04, 0x00, 0x04, 0x00, 0xa0, 0x0d, 0x02,
    0x03, 0x00, 0x91, 0xc8, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x30, 0x00,
    0x00, 0x00,
  ];
  ...
  fn test_build() {
    ...
    (&mut bytes[ETHER_HEADER_LEN + IPV4_HEADER_LEN + UDP_HEADER_LEN..108])
      .copy_from_slice(
        &FRAME_BYTES[ETHER_HEADER_LEN + IPV4_HEADER_LEN + UDP_HEADER_LEN..108],
      );
  ...
  fn pnet_build() {
    ...
    let mut bytes = [0xa5; 12];
    let mut udppkt =
      pnet::packet::udp::MutableUdpPacket::new(&mut bytes[..]).unwrap();
  ...
  fn run_test() -> (u16, [u8; 108]) {
    ...
    udppkt.adjust_ipv4_checksum(
      Ipv4Addr([192, 168, 29, 58]),
      Ipv4Addr([192, 168, 29, 160]),
    );
  ...
  fn pnet_test() -> (u16, [u8; 108]) {
    ...
    let mut udppkt =
      packet::udp::MutableUdpPacket::new(&mut bytes[34..108]).unwrap();
