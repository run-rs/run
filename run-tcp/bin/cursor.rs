...
impl run_tcp::Packet for RunTcpPacketProcesser {
  fn build(
    &mut self,
    ...
    match repr.ctrl {
      TcpControl::None => (),
      TcpControl::Psh => tcpheader.set_psh(true),
      TcpControl::Syn => tcpheader.set_syn(true),
      TcpControl::Fin => tcpheader.set_fin(true),
      TcpControl::Rst => tcpheader.set_rst(true),
    }
  ...
  fn parse(
    ...
    let total_packet_len =
      ippkt.packet_len() as usize + run_tcp::ETHER_HEADER_LEN;
...
fn server_start(args: &Flags) {
  ...
  run_tcp::poll(run, 0, &mut stack, run_tcp::OFFLOAD::NONE);
  jh.join().unwrap();
}
...
fn client_start(args: &Flags) {
  ...
  run_tcp::poll(run, 3, &mut stack, run_tcp::OFFLOAD::NONE);
