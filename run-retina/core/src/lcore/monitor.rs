...
impl Display {
  ...
  fn mempool_usage(&self, ports: &BTreeMap<PortId, Vec<RxQueue>>) {
    for name in ports.keys().map(|id| format!("mempool_{}", id.socket_id())) {
      let cname =
        CString::new(name.clone()).expect("Invalid CString conversion");
...
impl AggRxStats {
  ...
  fn collect(
    ...
    for (port_id, rx_queues) in ports.iter() {
      ...
      match PortStats::collect(*port_id) {
        Ok(port_stats) => {
          ...
          // dropped
          hw_dropped_pkts += match port_stats
            .stats
            .get("rx_phy_discard_packets")
          {
            Some(v) => *v,
            None => {
    ...
    Ok(AggRxStats {
      ...
      ingress_pkts,
      good_bits: (good_bytes + (PSFD_SIZE + IPG_SIZE + FCS_SIZE) * good_pkts)
        * 8,
      good_pkts,
      process_bits: (process_bytes
        + (PSFD_SIZE + IPG_SIZE + FCS_SIZE) * process_pkts)
        * 8,
      process_pkts,
      hw_dropped_pkts,
      sw_dropped_pkts,
...
impl Throughputs {
  ...
  fn new(curr_rx: AggRxStats, init_rx: AggRxStats, ems: f64) -> Self {
    Throughputs {
      avg_ingress_bps: (curr_rx.ingress_bits - init_rx.ingress_bits) as f64
        / ems
        * 1000.0,
      avg_ingress_pps: (curr_rx.ingress_pkts - init_rx.ingress_pkts) as f64
        / ems
        * 1000.0,
      avg_good_bps: (curr_rx.good_bits - init_rx.good_bits) as f64 / ems
        * 1000.0,
      avg_good_pps: (curr_rx.good_pkts - init_rx.good_pkts) as f64 / ems
        * 1000.0,
      avg_process_bps: (curr_rx.process_bits - init_rx.process_bits) as f64
        / ems
        * 1000.0,
      avg_process_pps: (curr_rx.process_pkts - init_rx.process_pkts) as f64
        / ems
        * 1000.0,
