...
use super::handshake::{
  Certificate, ClientDHParams, ClientECDHParams, ClientHello,
  ClientKeyExchange, ClientRSAParams, KeyShareEntry, ServerDHParams,
  ServerECDHParams, ServerHello, ServerKeyExchange, ServerRSAParams,
};
use super::Tls;
use crate::conntrack::conn::conn_info::ConnState;
use crate::conntrack::pdu::L4Pdu;
use crate::protocols::stream::{
  ConnParsable, ParseResult, ProbeResult, Session, SessionData,
};
...
impl Tls {
  ...
  pub(crate) fn parse_handshake_clienthello(
    &mut self,
            ...
            TlsExtension::SNI(ref v) => {
              if !v.is_empty() {
                ...
                client_hello.server_name =
                  Some(match std::str::from_utf8(sni) {
  ...
  fn parse_handshake_certificate(
    &mut self,
    content: &TlsCertificateContents,
    direction: bool,
  ) {
  ...
  fn parse_handshake_serverkeyexchange(
    &mut self,
    content: &TlsServerKeyExchangeContents,
  ) {
    log::trace!("SKE: {:?}", content);
    if let Some(cipher) = self.cipher_suite() {
      match &cipher.kx {
        TlsCipherKx::Ecdhe | TlsCipherKx::Ecdh => {
          if let Ok((_sig, ref parsed)) =
            parse_server_ecdh_params(content.parameters)
          {
              ...
              parsed.curve_params.params_content
            {
              ...
              self.server_key_exchange =
                Some(ServerKeyExchange::Ecdh(ecdh_params));
            };
          }
        }
        TlsCipherKx::Dhe | TlsCipherKx::Dh => {
          if let Ok((_sig, ref parsed)) =
            parse_server_dh_params(content.parameters)
          {
        ...
        TlsCipherKx::Rsa => {
          if let Ok((_sig, ref parsed)) =
            parse_server_rsa_params(content.parameters)
          {
  ...
  fn parse_handshake_clientkeyexchange(
    &mut self,
    content: &TlsClientKeyExchangeContents,
  ) {
    log::trace!("CKE: {:?}", content);
    if let Some(cipher) = self.cipher_suite() {
      match &cipher.kx {
        TlsCipherKx::Ecdhe | TlsCipherKx::Ecdh => {
          if let Ok((_rem, ref parsed)) =
            parse_client_ecdh_params(content.parameters)
          {
            let ecdh_params = ClientECDHParams {
              kx_data: parsed.ecdh_yc.point.to_vec(),
            };
            self.client_key_exchange =
              Some(ClientKeyExchange::Ecdh(ecdh_params));
          }
        }
        TlsCipherKx::Dhe | TlsCipherKx::Dh => {
          if let Ok((_rem, ref parsed)) =
            parse_client_dh_params(content.parameters)
          {
        ...
        TlsCipherKx::Rsa => {
          if let Ok((_rem, ref parsed)) =
            parse_client_rsa_params(content.parameters)
          {
  ...
  /// Parse a TLS message.
  pub(crate) fn parse_message_level(
    &mut self,
    msg: &TlsMessage,
    direction: bool,
  ) -> ParseResult {
