  ...
  S: Subscribable,
{
  ...
  pub(crate) fn run(&self) {
    ...
    let registry = ParserRegistry::build::<S>(&self.filter)
    ...
    let mut cap =
      Capture::from_file(pcap).expect("Error opening pcap. Aborting.");
  ...
  fn get_mempool_raw(&self) -> *mut dpdk::rte_mempool {
    let cname = CString::new(self.mempool_name.clone())
      .expect("Invalid CString conversion");
