...
impl PortStats {
  ...
  pub(crate) fn collect(port_id: PortId) -> Result<Self> {
    ...
    let len =
      unsafe { dpdk::rte_eth_xstats_get(port_id.raw(), table.as_mut_ptr(), 0) };
    if len < 0 {
    ...
    let nb_xstats = unsafe {
      dpdk::rte_eth_xstats_get(port_id.raw(), xstats.as_mut_ptr(), len as u32)
    };
    ...
    match (discards, total) {
      (Some(discards), Some(total)) => {
        ...
        if discard_rate > 0.0001 {
          println!(
            "Out of Buffer %: {}",
            discard_rate.to_string().bright_red()
          );
