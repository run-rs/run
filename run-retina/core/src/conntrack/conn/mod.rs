  ...
  T: Trackable,
{
  ...
  pub(super) fn new_tcp(
    ...
    let tcp_conn = if ctxt.flags & SYN != 0
      && ctxt.flags & ACK == 0
      && ctxt.flags & RST == 0
    {
  ...
  pub(super) fn update(
    ...
    match &mut self.l4conn {
      ...
      L4Conn::Udp(_udp_conn) => {
        self.info.consume_pdu(pdu, subscription, registry)
      }
    }
  }
  ...
  pub(crate) fn terminate(
    ...
    match self.info.state {
      ConnState::Probing => {
        if let FilterResult::MatchTerminal(_) =
          subscription.filter_conn(&self.info.cdata)
        {
      ...
      ConnState::Parsing => {
        ...
        for session in self.info.cdata.conn_parser.drain_sessions() {
          if subscription
            .filter_session(&session, self.info.cdata.conn_term_node)
          {
            if session.id == 0 {
