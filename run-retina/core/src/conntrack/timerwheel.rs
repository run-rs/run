...
impl TimerWheel {
  ...
  pub(super) fn new(max_timeout: usize, timeout_resolution: usize) -> Self {
    if timeout_resolution > max_timeout {
      panic!(
        "Timeout check period must be smaller than maximum inactivity timeout"
      )
    }
  ...
  pub(super) fn insert(
    ...
    let timer_index =
      ((current_time + inactivity_window) / self.period) % self.timers.len();
  ...
  pub(super) fn remove_inactive<T: Trackable>(
    ...
    let check_time =
      (now - self.start_ts).as_millis() as usize / period * period;
    ...
    for expire_bucket in self.next_bucket..last_expire_bucket {
      ...
      for conn_id in list.drain(..) {
          ...
          table.raw_entry_mut().from_key(&conn_id)
        {
          ...
          let last_seen_time =
            (conn.last_seen_ts - self.start_ts).as_millis() as usize;
