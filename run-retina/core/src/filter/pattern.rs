...
impl FlatPattern {
  ...
  pub(super) fn is_fully_qualified(&self) -> bool {
    ...
    let mut ret = true;
    let mut prev_header =
      unwrap_or_ret_false!(labels.get_by_right(&protocol!("ethernet")));
      ...
      match labels.get_by_right(header) {
        Some(node) => {
          ...
          let node_path: HashSet<Vec<NodeIndex>> =
            algo::all_simple_paths(&layers, *node, *ethernet, 0, None)
              .collect();
    ...
    for fq_path in fq_paths {
      ...
      if headers.is_subset(&fq_headers) {
        ...
        for protocol in fq_path.iter() {
          ...
          let mut proto_predicates =
            proto_predicates.into_iter().collect::<Vec<_>>();
          proto_predicates.sort();

          assert!(
            fq_pattern.add_protocol(protocol.to_owned(), proto_predicates)
          );
        }
...
impl LayeredPattern {
  ...
  fn add_protocol(
    ...
    field_predicates: Vec<Predicate>,
  ) -> bool {
    // temp placeholders
    let layers = &*LAYERS;
    let labels = &*NODE_BIMAP;
  ...
  pub(super) fn get_header_predicates(
    &self,
  ) -> &LinkedHashMap<ProtocolName, Vec<Predicate>> {
    &self.0
  }
