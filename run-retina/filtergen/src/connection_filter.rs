...
pub(crate) fn gen_connection_filter(
    ...
    let connection_filter = quote! {
        match conn.pkt_term_node {
            #( #body )*
            _ => return core::filter::FilterResult::NoMatch,
        }
    };
...
fn add_node_match_arm(
    ...
    if node.is_terminal {
        ...
        code.push(quote! {
            #idx_lit => return core::filter::FilterResult::MatchTerminal(#idx_lit),
        })
    } else {
...
fn add_service_pred(
    ...
    if matches!(node.terminates, Terminate::Connection) {
        ...
        if node.is_terminal {
            code.push(quote! {
                if matches!(conn.service(), core::protocols::stream::ConnParser::#service_ident { .. }) {
