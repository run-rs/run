...
//! module](../core/protocols) of the same name. The [`filter`](macro@self::filter) macro
...
//!
//! For example, [`ipv4`](../core/protocols/packet/ipv4) and
//! [`tls`](../core/protocols/stream/tls) are filterable protocols because they are both
...
//!
//! For example,
//! [`ipv4.src_addr`](../core/protocols/packet/ipv4/struct.Ipv4.html#method.src_addr) and
//! [`tls.sni`](../core/protocols/stream/tls/struct.Tls.html#method.sni) are both filterable
...
pub fn filter(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let packet_filter_fn = quote! {
        #[inline]
        fn packet_filter(mbuf: &core::Mbuf) -> core::filter::FilterResult {
            #packet_filter_body
        }
    };
    ...
    let connection_filter_fn = quote! {
        #[inline]
        fn connection_filter(conn: &core::protocols::stream::ConnData) -> core::filter::FilterResult {
    ...
    let session_filter_fn = quote! {
        #[inline]
        fn session_filter(session: &core::protocols::stream::Session, idx: usize) -> bool {
            #session_filter_body
        }
    };

    let filtergen = quote! {
        fn filter() -> core::filter::FilterFactory {
            #packet_filter_fn
            #connection_filter_fn
            #session_filter_fn
            core::filter::FilterFactory::new(#filter_str, packet_filter, connection_filter, session_filter)
        }
