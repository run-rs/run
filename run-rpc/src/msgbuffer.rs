...
use log::trace;
use run_packet::{
  ether::ETHER_HEADER_LEN, ipv4::IPV4_HEADER_LEN, udp::UDP_HEADER_LEN,
  CursorMut, PktBuf, PktMut,
};
...
pub const HEADER_LEN: usize =
  ETHER_HEADER_LEN + UDP_HEADER_LEN + IPV4_HEADER_LEN + RPC_HEADER_LEN;
...
impl MsgBuffer {
  ...
  pub fn EMPTY() -> Self {
    Self(Rc::new(RefCell::new(MsgBufferInner {
      buf: None,
      data_size: 0,
      max_data_size: 0,
      num_pkts: 0,
  ...
  pub fn ALLOCA_MSG(max_data_size: usize) -> Self {
    ...
    let buf_ptr =
      unsafe { NonNull::new_unchecked(alloc_zeroed(layout.clone())) };
    Self(Rc::new(RefCell::new(MsgBufferInner {
      buf: unsafe {
  ...
  pub fn copy_data_from_buf<T: PktBuf>(&mut self, buf: &mut T, pkt_idx: usize) {
    ...
    let slice = unsafe { std::slice::from_raw_parts_mut(ptr, buf.remaining()) };
  ...
  pub unsafe fn from_pkthdr(header: &RpcHeader, max_data_size: usize) -> Self {
    Self(Rc::new(RefCell::new(MsgBufferInner {
      buf: Some(NonNull::new_unchecked(
        (header as *const _ as *mut u8).add(HEADER_LEN),
      )),
      data_size: max_data_size,
      max_data_size: max_data_size,
      num_pkts: 1,
  ...
  pub fn resize_msg_buffer(&mut self, new_data_size: usize) {
    ...
    trace!(
      "Resize msg buffer: new data size :{} new num pkts:{}",
      new_data_size,
      (new_data_size + TTR_MAX_DATA_PER_PKT - 1) / TTR_MAX_DATA_PER_PKT
    );
  }
  ...
  pub fn get_pkthdr_0(&self) -> &mut RpcHeader {
    unsafe {
      &mut *(self
        ...
        .unwrap()
        .as_ptr()
        .sub(HEADER_LEN) as *mut RpcHeader)
    }
  }
  ...
  pub fn get_pkthdr_n(&self, idx: usize) -> &mut RpcHeader {
    ...
    unsafe {
      ...
      let offset = self
        ...
        .unwrap()
        .as_ptr()
        .add(max_data_size + (idx - 1) * HEADER_LEN);
  ...
  pub fn get_buf_n<'a>(&'a self, idx: usize) -> MsgSegBuffer<'a> {
    ...
    let header = if idx == 0 {
      ...
      unsafe {
        ...
        std::slice::from_raw_parts_mut(offset, HEADER_LEN)
      }
    };
    MsgSegBuffer {
      header: header,
  ...
  fn trim_off(&mut self, cnt: usize) {
    ...
    if cnt + self.cursor <= self.header.len() {
      ...
      self.buf = unsafe {
        std::slice::from_raw_parts_mut(
          self.buf.as_mut_ptr(),
          cnt + self.cursor - self.header.len(),
        )
      };
    }
  }
...
impl RpcHeader {
  ...
  pub fn buf(&self) -> &[u8] {
    unsafe {
      std::slice::from_raw_parts(self.headroom.as_ptr().add(42), RPC_HEADER_LEN)
    }
  }
  ...
  pub fn from_slice(slice: &[u8]) -> Self {
    ...
    ret
      .buf_mut()
      .copy_from_slice(&slice[42..std::mem::size_of::<Self>()]);
    ret
  }
...
impl<T: PktBuf> RpcPkt<T> {
  ...
  pub fn owned_header(&self) -> RpcHeader {
    ...
    ret
      .buf_mut()
      .copy_from_slice(&self.buf.chunk()[0..RPC_HEADER_LEN]);
    ret
  }
}
...
mod test {
  ...
  fn rpc_header_api_test() {
    let msg_buf = MsgBuffer::ALLOCA_MSG(HEADER_LEN);
