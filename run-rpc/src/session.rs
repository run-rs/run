...
impl SmPkt {
  ...
  pub(crate) fn set_pkt_type(&mut self, pkt_type: SmPktType) {
    NetworkEndian::write_i32(&mut self.buf_[field::PKT_TYPE], pkt_type.into());
  }
  ...
  pub(crate) fn set_err_type(&mut self, err_type: SmErrType) {
    NetworkEndian::write_i32(&mut self.buf_[field::ERR_TYPE], err_type.into());
  }
  ...
  pub(crate) fn set_client_info(&mut self, sep: &SessionEndpoint) {
    ...
    NetworkEndian::write_u16(
      &mut self.buf_[field::CLIENT_SESSION_NUM],
      sep.session_num,
    );
    NetworkEndian::write_u16(
      &mut self.buf_[field::CLIENT_UDP_PORT],
      sep.sm_udp_port,
    );
    let _ = &mut self.buf_[field::CLIENT_ROUTING_INFO]
      .copy_from_slice(sep.routing_info.buf());
  }

  pub(crate) fn set_server_info(&mut self, sep: &SessionEndpoint) {
    (self.buf_[field::SERVER_RPC_ID.start]) = sep.rpc_id;
    NetworkEndian::write_u16(
      &mut self.buf_[field::SERVER_SESSION_NUM],
      sep.session_num,
    );
    NetworkEndian::write_u16(
      &mut self.buf_[field::SERVER_UDP_PORT],
      sep.sm_udp_port,
    );
    let _ = &mut self.buf_[field::SERVER_ROUTING_INFO]
      .copy_from_slice(sep.routing_info.buf());
  }
  ...
  pub(crate) fn is_req(&self) -> bool {
    match self.get_pkt_type() {
      SmPktType::ConnectReq | SmPktType::DisconnectReq | SmPktType::PingReq => {
        true
      }
      _ => false,
    }
