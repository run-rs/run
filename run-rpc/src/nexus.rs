...
use run_dpdk::{Mempool, RxQueue, TxQueue};
use run_packet::ether::MacAddr;
use run_packet::ipv4::Ipv4Addr;

use crate::constant::{
  RPC_REQUEST_TIMEOUT, SESSION_REQ_WINDOW, TTR_UNSIG_BATCH,
};
use crate::msgbuffer::{MsgBuffer, HEADER_LEN};
use crate::rpc::{ReqFunc, Rpc, RpcContext};
use crate::sslot::SSlot;
use crate::time::{ms_to_cycles};
...
impl Nexus {
  pub fn new() -> Result<Self> {
    ...
    let nexus_inner = Box::new(NexusInner {
      freq_ghz: freq_ghz,
  ...
  pub fn create_rpc(
    ...
    let mut sslot_arr: [Option<Rc<RefCell<SSlot>>>; SESSION_REQ_WINDOW] =
      unsafe { std::mem::zeroed() };
    let mut ctrl_msgbufs: [Option<MsgBuffer>; TTR_UNSIG_BATCH * 2] =
      unsafe { std::mem::zeroed() };
    let mut sslot_free_vec = ArrayVec::new();
    for i in 0..SESSION_REQ_WINDOW {
      if client {
        sslot_arr[i] = Some(Rc::new(RefCell::new(SSlot::client(i))));
        sslot_free_vec.push(i);
      } else {
        sslot_arr[i] = Some(Rc::new(RefCell::new(SSlot::server(i))));
        sslot_free_vec.push(i);
      }
    }
