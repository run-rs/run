...
fn smol_build(buf: &mut [u8], payload_len: usize) {
  let mut frame =
    wire::EthernetFrame::new_unchecked(&mut buf[0..42 + payload_len]);
  frame
    .set_src_addr(wire::EthernetAddress([0x00, 0x50, 0x56, 0xae, 0x76, 0xf5]));
  frame
    .set_dst_addr(wire::EthernetAddress([0x00, 0x0b, 0x86, 0x64, 0x8b, 0xa0]));
