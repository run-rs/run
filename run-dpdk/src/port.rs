...
impl PortInfo {
  pub(crate) unsafe fn try_get(port_id: u16) -> Result<Self> {
    ...
    let res = ffi::rte_eth_dev_info_get(
      port_id,
      &mut dev_info as *mut ffi::rte_eth_dev_info,
    );
    if res != 0 {
    ...
    let res = ffi::rte_eth_macaddr_get(
      port_id,
      &mut eth_addr as *mut ffi::rte_ether_addr,
    );
    if res != 0 {
...
impl PortConf {
  ...
  pub fn from_port_info(port_info: &PortInfo) -> Result<Self> {
    ...
    if Self::RTE_ETHER_MTU < port_info.min_mtu()
      || Self::RTE_ETHER_MTU > port_info.max_mtu()
    {
...
impl Port {
  pub(crate) fn try_create(
    ...
    let eth_conf = unsafe {
      port_conf.rte_eth_conf(rxq_confs.len() as u16, txq_confs.len() as u16)
    };
    let res = unsafe {
