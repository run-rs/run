...
impl Mempool {
  ...
  pub fn fill_batch<const N: usize>(&self, batch: &mut ArrayVec<Mbuf, N>) {
    ...
    unsafe {
      ...
      let alloc_nb = ffi::rte_pktmbuf_alloc_bulk_(
        self.ptr.as_ptr(),
        mbufs,
        (N - batch_len) as u32,
      );
  ...
  pub(crate) fn try_create(
    ...
    let data_room_size =
      conf.dataroom.checked_add(Self::MBUF_HEADROOM).ok_or(err)?;
    let socket_id = i32::try_from(conf.socket_id).map_err(|_| err)?;

    // create the mempool
    let cname = CString::new(mpool_name)
      .map_err(|_| Error::service_err("invalid mempool name"))?;
    let raw = unsafe {
