    ...
    measure_fraq(PRECISION, 50).and_then(
      |(nanos_per_cycle, cycles_per_sec, ref_time, ref_tsc)| {
        ...
        for (idx, cpu) in cpus.iter().enumerate() {
          ...
          jhs.push(std::thread::spawn(move || {
            ...
            let (tl_nanos_per_cycle, _, tl_ref_time, tl_ref_tsc) =
              measure_fraq(PRECISION, 50).unwrap_or((
                0.0,
                0,
                Instant::now(),
                tsc(),
            ...
            *rwl.write().unwrap() =
              (tl_nanos_per_cycle, tl_ref_time, tl_ref_tsc);
          }));
        }
        for jh in jhs {
          jh.join().unwrap();
        ...
        tls
          .iter()
          .map(|rwl| {
            let (tl_nanos_per_cycle, tl_ref_time, tl_ref_tsc) =
              *rwl.read().unwrap();
